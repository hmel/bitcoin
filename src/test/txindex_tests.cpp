...
{
    TxIndex txindex(m_args.GetDataDirNet(), 1 << 20, true);
    ...
    for (const auto& txn : m_coinbase_txns) {
        BOOST_CHECK(!txindex.FindTx(txn->GetHash(), block_hash, tx_disk, m_args));
    }
    ...
    for (const auto& txn : m_coinbase_txns) {
        if (!txindex.FindTx(txn->GetHash(), block_hash, tx_disk, m_args)) {
