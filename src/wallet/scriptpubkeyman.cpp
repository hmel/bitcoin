...
bool LegacyScriptPubKeyMan::TopUpInactiveHDChain(const CKeyID seed_id, int64_t index, bool internal)
{
    ...
    // Top up key pool
    int64_t target_size = std::max(m_args.GetIntArg("-keypool", DEFAULT_KEYPOOL_SIZE), (int64_t) 1);
...
bool LegacyScriptPubKeyMan::TopUp(unsigned int kpSize)
{
    ...
    }
    {
        ...
        if (kpSize > 0)
            ...
            nTargetSize = std::max(m_args.GetIntArg("-keypool", DEFAULT_KEYPOOL_SIZE), (int64_t) 0);
...
bool DescriptorScriptPubKeyMan::TopUp(unsigned int size)
{
    ...
    if (size > 0) {
        ...
        target_size = std::max(m_args.GetIntArg("-keypool", DEFAULT_KEYPOOL_SIZE), (int64_t) 1);
    }
