                       ...
                       TxVerbosity tx_verbosity)
{
    ...
    case RetFormat::JSON: {
        UniValue objBlock = blockToJSON(block, tip, pblockindex, tx_verbosity, Params().args());
...
static bool rest_mempool_info(const std::any& context, HTTPRequest* req, const std::string& strURIPart)
{
    ...
    switch (rf) {
    case RetFormat::JSON: {
        UniValue mempoolInfoObject = MempoolInfoToJSON(*mempool, Params().args());
...
static bool rest_tx(const std::any& context, HTTPRequest* req, const std::string& strURIPart)
{
    ...
    const CTransactionRef tx = GetTransaction(/* block_index */ nullptr, node->mempool.get(), hash, Params().GetConsensus(), hashBlock, *node->args);
    if (!tx) {
        return RESTERR(req, HTTP_NOT_FOUND, hashStr + " not found");
    }
    ...
    switch (rf) {
    case RetFormat::BINARY: {
        CDataStream ssTx(SER_NETWORK, PROTOCOL_VERSION | RPCSerializationFlags(Params().args()));
        ssTx << tx;
