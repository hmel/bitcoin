...
void TorController::add_onion_cb(TorControlConnection& _conn, const TorControlReply& reply)
{
    if (reply.code == 250) {
        ...
        service = LookupNumeric(std::string(service_id+".onion"), BaseParams().GetDefaultPort());
...
void TorController::auth_cb(TorControlConnection& _conn, const TorControlReply& reply)
{
    if (reply.code == 250) {
        ...
        _conn.Command(strprintf("ADD_ONION %s Port=%i,%s", private_key, BaseParams().GetDefaultPort(), m_target.ToStringIPPort()),
